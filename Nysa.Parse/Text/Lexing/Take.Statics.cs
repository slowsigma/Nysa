...
namespace Nysa.Text.Lexing
{
    ...
    public static partial class Take
    {
        ...
        {
            var actuals = alternates.Where(a => a is Some<Node>).Select(s => ((Some<Node>)s).Value);
        ...
        public static Option<LongestNode> Longest(IEnumerable<Option<Node>> alternatives, params Option<Node>[] additional)
        {
            ...
            return (nodes.Length > 1) ? (new LongestNode(nodes)).Some() : Option.None;
        }
        ...
        public static Option<AnyOneNode> AnyOne(this String alternatives, Boolean? ignoreCase = null)
        {
            ...
            return (nodes.Length > 1) ? (new AnyOneNode(nodes, ignoreCase.GetValueOrDefault(Take.IgnoreCase))).Some() : Option.None;
        }
        ...
        public static IEnumerable<Hit> Repeat(this SeekNode seek, String source, Boolean includeSkips = false, Identifier? endOfInputId = null)
        {
            ...
            if ((curr.Position + curr.Length >= source.Length) && endOfInputId.HasValue && endOfInputId.Value != Identifier.None)
                yield return new Hit(new TextSpan(source, source.Length, 0), endOfInputId.Value);
        }
